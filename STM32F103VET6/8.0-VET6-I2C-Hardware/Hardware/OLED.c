...
/**
  ...
  * 返 回 值：无
  */
uint8_t cmds[]={0xAE,0xD5, 0x80,0xA8, 0x3F,0xD3, 0x00,0x40,0xA1,0xC8,0xDA, 0x12,0x81, 0xCF,0xD9, 0xF1,
0xDB, 0x30,0xA4,0xA6,0x8D, 0x14,0xAF};
void OLED_WriteCommand(uint8_t *Command,uint16_t num)
{
	while(hi2c2.State!=HAL_I2C_STATE_READY);
	HAL_I2C_Mem_Write_DMA(&hi2c2,0x78,0x00,I2C_MEMADD_SIZE_8BIT, Command,num);
}
...
{
	while(hi2c2.State!=HAL_I2C_STATE_READY);
	HAL_I2C_Mem_Write_DMA(&hi2c2,0x78,0x40,I2C_MEMADD_SIZE_8BIT, Data,Count);

}
//	HAL_I2C_Master_Transmit(&hi2c2, 0x78, buffer, Count + 1, 100);
...
void OLED_Init(void)
{
	...
	{
			OLED_WriteCommand((uint8_t *)&cmds[i], 1);
		HAL_Delay(10);
	}
...
void OLED_Update(void)
{
		...
		OLED_WriteData(*OLED_DisplayBuf,1024);
